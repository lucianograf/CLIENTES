...
/*/
User Function zzXmlNfeSef(cTipo,cSerie,cNota,cClieFor,cLoja,cNotaOri,cSerieOri)
		...
		Else				
			...
			While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
				...
				If !Empty((cAliasSD1)->D1_NFORI)
					...
					// Força o preenchmimento - Customização Grupo forta
					/*aadd(aNfVinc,{CTOD("12/02/2020"),;//SD2->D2_EMISSAO,
								  "1  ",;//SD2->D2_SERIE,
								  "004163   ",;//SD2->D2_DOC,
								  SM0->M0_CGC,;
								  SM0->M0_ESTCOB,;
								  "SPED",;//SF2->F2_ESPECIE,
								  "35200226528614000134550010000041631100169805",;//SF2->F2_CHVNFE,
								  0,;
								  " ",;//SD2->D2_PEDIDO,,;
								  "N"})//SF2->F2_TIPO})
					*/
...
Do Case
	...
	Case (cVeramb >= "3.10" .and. (!Empty(aNfVinc) .Or. !Empty(aRefECF) .Or. !Empty(aNfVincRur))) .and. ( (aNota[5] $ "D|B" .And. SF4->F4_AJUSTE <> "S") .or. (aNota[5] $ "N" .And. SF4->F4_PODER3 == "D") )
     ...
     documento fiscal referenciado conforme  NT 2013/005 v 1.20.
   	 */
    Case (aNota[5]) $ "D" .and. Empty(aNfVinc).and. Alltrim(SFT->FT_CFOP) $ "1201-1202-1410-1411-5921-6921-1661"
   	 	cTPNota:= "4" 
