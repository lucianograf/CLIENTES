...
//---------------------------------------------------------------------------------------
// Analista   : Marcelo Alberto Lauschner - 07/07/2011
// Nome função: LoadDicSped
// Parametros : cTable
// Objetivo   : Criação da Tabela CONDORTMKC
// Retorno    : 
// Alterações : 
//---------------------------------------------------------------------------------------
...
Do Case
	Case cTable == "CONDORTMKC"
		...
		Aadd(aCampos,{"CTC_FILIAL","C",004,0})	// Código da Filial
		...
		Aadd(aIndices,{"CTC_EMPRES+CTC_FILIAL+CTC_CLIENT+CTC_LOJA","01"})
		Aadd(aIndices,{"CTC_EMPRES+CTC_FILIAL+DTOS(CTC_DATA)","02"})
		Aadd(aIndices,{"CTC_EMPRES+CTC_FILIAL+DTOS(CTC_ULTACF)","03"})
...
DbSetOrder(2)
DbSeek("E1_HIST")
nTmE1Hist	:= SX3->X3_TAMANHO
If SX3->X3_TAMANHO <> nTmE1Hist
	RecLock("SX3",.F.)
	SX3->X3_TAMANHO	:= nTmE1Hist
	MsUnlock()
	aAdd(aArqUpd,"SE1")
	cAlias += "SE1"
Endif
