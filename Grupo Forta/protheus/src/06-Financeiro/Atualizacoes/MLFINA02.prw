...
/*/{Protheus.doc} MLFINA02
(Geração de arquivo de Remessa Sispag Itau)
...
User Function MLFINA02()

	Local		lContinua	:= .F.
	Local		oDlg1
	Local		cDadosMv	:= sfCriaSx6()
	Private		cCodA6		:= "341"
	Private		cAgeA6		:= Substr(cDadosMv,1,4)
	Private		cConA6		:= Substr(cDadosMv,5,10)
	Private 	cSubEE		:= Substr(cDadosMv,15,3)
	Private 	cArq 		:= Space(30)
	Private 	cBord 		:= Space(6)
	
	If Empty(cDadosMv)
		MsgAlert("O parâmetro GM_FINA02A não está configurado corretamente. Favor preencher com os dados de Agência+Conta+Sub-Conta(AAAA+CCCCCCCCCC+SSS)")
		Return 
	Endif
	
	dbSelectArea("SEE")
	dbSetOrder(1)
	If DbSeek(xFilial("SEE")+ cCodA6+cAgeA6+cConA6+cSubEE)
	
	Else
		MsgAlert("Não foi encontrada a configuração de Parâmetros para a conta '341-"+cAgeA6+"-"+cConA6+"' ")
		Return .F.
	Endif
	
	
	@ 200,1 TO 380,395 DIALOG oDlg1 TITLE OemToAnsi("Envio de Borderô - Frimazo")
	@ 02,10 TO 070,190
...
Static Function sfCriaSx6()
	...
	DbSelectArea("SX6")
	DbSetOrder(1)
	If DbSeek(cFilAnt + "GF_FINA02A")
		cRetMv	:= GetMv("GF_FINA02A")
	Else
		RecLock("SX6",.T.)
		SX6->X6_FIL		:= cFilAnt
		SX6->X6_VAR		:= "GF_FINA02A"
		SX6->X6_TIPO	:= "C"
		SX6->X6_DESCRIC	:= "Agência / Conta / Sub conta     "
		SX6->X6_DSCSPA	:= ""
		SX6->X6_DSCENG	:= ""
		SX6->X6_DESC1	:= "para geração de arquivo de remessa Itaú           "
		SX6->X6_DSCSPA1	:= ""
		SX6->X6_DSCENG1	:= ""
		SX6->X6_DESC2	:= "Sispag                                            "
		SX6->X6_DSCSPA2	:= ""
...
Static Function sfExec()
	...
	Else
		...
		MsgAlert("Não foi encontrada a configuração de Parâmetros para a conta '341-1293-25998' ")
		Return .F.
	Endif
	...
	cCpo += StrZero(Val(SEE->EE_CONTA),12)                              			// 	059-070 9(12) Conta
	cCpo += Space(1)    															// 	071-071 X(01) Branco
	cCpo += Substr(SEE->EE_DVCTA,1,1)                                   			//	072-072 9(01) Digitocc
	...
	For x := 1 To Len(aDoc)
		...
		If dbSeek(xFilial("SE2")+aDoc[x][1]+aDoc[x][2]+aDoc[x][3]+aDoc[x][4]+aDoc[x][5]+aDoc[x][6])
			...
			If dbSeek(xFilial("SA2")+SE2->E2_FORNECE+SE2->E2_LOJA)
				...
				If Alltrim(SE2->E2_NATUREZ)=="SALARIOS" .And. SA2->A2_BANCO == '341'     ////Tipo de Pagamento
					cTipPg := "30"
				Else
					cTipPg := "20"
				Endif
				...
				If SA2->A2_BANCO == '341'//Forma de Pagamento ITAU
					cForPg := "01"
				ElseIf SE2->E2_SALDO > 0	//VER QUESTAO DO ACRES E DECRES // O valor anterior era de R$ 5.000,00
	...
	For x:=1 To Len(aTit)
		...
		If dbSeek(xFilial("SE2")+aTit[x][1]+aTit[x][2]+aTit[x][3]+aTit[x][4]+aTit[x][5]+aTit[x][6])
			...
			If DbSeek(xFilial("SA2")+SE2->E2_FORNECE+SE2->E2_LOJA)
				...
				If SA2->(FieldPos("A2_XJ52COD")) > 0 .And. SA2->(FieldPos("A2_XJ52LOJ")) > 0 .And. !Empty(SA2->A2_XJ52COD+SA2->A2_XJ52LOJ)
					DbSelectArea("SA2")
					DbSetOrder(1)
...
Static Function sfDetTit()
	...
	cCpo += Strzero((aDetTit[nX3,6]*100),15) 												// 	100-114 9(013)V9(02) Saldo
