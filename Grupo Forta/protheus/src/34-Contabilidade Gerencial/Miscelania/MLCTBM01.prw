...
User Function MLCTBM01()
	...
	Private oMulti := MsNewGetDados():New(034, 005, 226, 415,GD_INSERT+GD_DELETE+GD_UPDATE,"U_MLCTBMV2()"/*cLinhaOk*/,;
	"AllwaysTrue()"/*cTudoOk*/,"+CT2_LINHA",;
	,4/*nFreeze*/,10000/*nMax*/,"U_MLCTBMV1()"/*cCampoOk*/,/*cSuperApagar*/,;
	...
	If nRefrBox == 2 .And. cArqImp <> Nil .And. File(cArqImp)
		...
		While !Eof()
			...
			If Len(aArrDados) == 5 .And. Alltrim(aArrDados[3]) == "Colaborador:"
				...
				cQry := "SELECT A2_COD,A2_LOJA,A2_NOME,A2_CC,A2_NREDUZ,A2_XCODRUB "
				cQry += "  FROM "+RetSqlName("SA2")
				cQry += " WHERE D_E_L_E_T_ = ' ' "
				cQry += "   AND A2_MSBLQL <> '1' "
				cQry += "   AND A2_XCODRUB = '"+StrZero(Val(aArrDados[4]),6) + "' "
			...
			ElseIf Len(aArrDados) == 3 .And. Alltrim(aArrDados[1]) == "Colaborador:"
				...
				lContinua	:= .T. 
				If Select("QSA2") > 0
					If StrZero(Val(QSA2->A2_XCODRUB),6) <> StrZero(Val(aArrDados[2]),6)
						QSA2->(DbCloseArea())
					Else
						lContinua	:= .F. 
					Endif
				...
				If lContinua
					...
					cQry += "   AND A2_MSBLQL <> '1' "
					IF SM0->M0_CODIGO <> '06'
						cQry += "   AND LPAD(TRIM(A2_XCODRUB),6,'0') = '"+StrZero(Val(aArrDados[2]),6) + "' "
					ELSE
						cQry += "   AND A2_XCODRUB = '"+StrZero(Val(aArrDados[2]),6) + "' "
					ENDIF
					cQry += "   AND A2_FILIAL = '"+xFilial("SA2")+"' "
				...
				For nI := 1 To Len(aHeader)
					...
					ElseIf ( aHeader[nI][10] <> "V") .AND. (aHeader[nI][08] <> "M")
						...
						ElseIf Alltrim(aHeader[nI][2]) == "CT2_HIST"
							aCols[Len(aCols)][nI]	:= CriaVar(aHeader[nI][2],.T.)
							aCols[Len(aCols)][nI]		:= aArrDados[1]+"-"+aArrDados[2]+"- Cód."+QSA2->A2_XCODRUB
						...
						ElseIf Alltrim(aHeader[nI][2]) == "CT2_ORIGEM"
							aCols[Len(aCols)][nI] := Padr(QSA2->A2_XCODRUB+"-"+QSA2->A2_NREDUZ,TamSX3(aHeader[nI][2])[1])
						Else
							aCols[Len(aCols)][nI] := CriaVar(aHeader[nI][2],.T.)
						Endif
					Else
			...
			ElseIf Len(aArrDados) == 19
				...
				cQry += " WHERE D_E_L_E_T_ = ' ' "
				cQry += "   AND A2_MSBLQL <> '1' "
				cQry += "   AND A2_XCODRUB = '"+StrZero(Val(aArrDados[1]),6) + "' "
...
Static Function sfCadConv()
	...
	Private nPxConta	    := 4
	DbSeek("CT1_DESC01")
	Aadd(aHeadConv,{"Descrição Conta"			,	"DESCCONTA"	,	SX3->X3_PICTURE		,	SX3->X3_TAMANHO,	SX3->X3_DECIMAL,"",,	"C","",""})
	Private nPxDescCT	    := 5
	...
	,1/*nFreeze*/,10000/*nMax*/,"U_MLCTBMV1()"/*cCampoOk*/,/*cSuperApagar*/,;
...
Static Function sfMontaCols(aColsConv)
	...
	cQry += "  FROM TOP_FIELD "
	cQry += " WHERE FIELD_TABLE LIKE '%CONDOR_CONV_CTB_RUBI%' "
	...
	If QRLG->NREG == 0
		...
		cQry := 'DROP TABLE CONDOR_CONV_CTB_RUBI'
		
		TcSQLExec(cQry)
		
		
		cQry := 'CREATE TABLE CONDOR_CONV_CTB_RUBI '
		cQry += '  ("CRC_EMP" 		CHAR('+cValToChar(Len(cEmpAnt))+') NOT NULL,'
		cQry += '	"CRC_FIL"		CHAR('+cValToChar(Len(cFilAnt))+') NOT NULL,'
...
Static Function sfGravaConv()
	...
	For nX := 1 To Len(oConvGet:aCols)
		...
		If !oConvGet:aCols[nX,Len(oConvGet:aHeader)+1] .And. !Empty(oConvGet:aCols[nX,nPxCod])
			...
			Begin Transaction 
				lResult := TCSQLEXEC(cQry)
				If lResult < 0
					Return MsgStop("Erro: " + TCSQLError())
				EndIf 
			End Transaction 
		Else
			cQry := "SELECT COUNT(*) NEXIST "
