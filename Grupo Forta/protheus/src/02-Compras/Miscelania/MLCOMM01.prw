...
Static Function sfCarrega(aCols,aHeader,nRefrBox)
	...
	Local	nUsado		:=  0
	Local	aCpo		:=  { "D1_ITEM","D1_COD","D1_QUANT","D1_VUNIT","D1_TOTAL","D1_VALDESC","D1_ZVLINV","D1_TES","D1_SEGURO","D1_DESPESA","D1_VALFRE","D1_ALIQII",;
	...
	"CD5_VAFRMM","CD5_VLRIOF","CD5_VAFRMM","CD5_CNPJAE","CD5_UFTERC"}
	Local	aCpoAux			:= {}
	Local	aColsAdd		:= {}
	Local	cUsado			:= ""
	...
	For iX := 1 To Len(aCpo)
		If DbSeek(aCpo[iX])
			...
			SX3->X3_F3 		,;
			SX3->X3_CONTEXT,;
			Iif(Substr(SX3->X3_CBOX,1,1) == "#",&(Substr(SX3->X3_CBOX,2)),SX3->X3_CBOX) 	,;
			""}) //SX3->X3_RELACAO })
			nUsado++
			If aCpo[iX] == "D1_COD"
				cUsado	:= SX3->X3_USADO
			Endif
			&("n"+StrTran(SX3->X3_CAMPO,"_",""))	:= nUsado
		Endif
	Next
...
Static Function sfGrvDados 
	...
	For nForG := 1 To Len(oMulti:aCols)
		...
		If !oMulti:aCols[nIX,Len(oMulti:aHeader)+1] .And. !Empty(oMulti:aCols[nIX,aScan(oMulti:aHeader,{|x| Alltrim(x[2]) == "D1_COD" })])
			...
			Aadd(aLinha,{"D1_ZVLINV"  	, oMulti:aCols[nIX,aScan(oMulti:aHeader,{|x| Alltrim(x[2]) == "D1_ZVLINV" })]			,Nil,Nil})
