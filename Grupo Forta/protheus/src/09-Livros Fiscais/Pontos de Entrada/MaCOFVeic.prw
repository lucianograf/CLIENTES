...
/*/{Protheus.doc} MaCofVeic
//Ponto de Entrada para Retornar Nova Base de Cálculo, alíquota e valor do Cofins
...
User function MaCofVeic()
	...
	Local		nCf2Item	:= ParamIxb[1]
	Local		nCf2Base	:= ParamIxb[2]
	Local		nCf2Aliq	:= ParamIxb[3]
	Local		nCf2Vlr		:= ParamIxb[4]
	Local		nCf2NewBs	:= nCf2Base
	Local		aRet		:= {nCf2Base,nCf2Aliq,nCf2Vlr} // Monta vetor para devolver igual ao recebido se não for tratado no PE
	Local		cCodPrd		:= MaFisRet(nCf2Item,"IT_PRODUTO")
	Local		nPrdQte		:= MaFisRet(nCf2Item,"IT_QUANT")
	Local		cNFOPERNF	:= MaFisRet(,"NF_OPERNF")
	Local		cNFCLIFOR	:= MaFisRet(,"NF_CLIFOR")
	Local		cIT_TES		:= MaFisRet(nCf2Item,"IT_TS")
	/*
	If "PS2" $ cTipo
	...
	*/
	...
	If cNFOPERNF == "S"  .And. cNFCLIFOR == "C" // Para a Saída
		...
		nCf2NewBs	:= sfCalNewBs(cCodPrd,nCf2Base,nPrdQte)

		//MsgAlert("Produto : " + cCodPrd + " Base: " + cValToChar(nCf2Base) + " Aliquota " + cValToChar(nCf2Aliq) + " Valor " + cValToChar(nCf2Vlr) + " NewBs " + cValToChar(nCf2NewBs))
		If nCf2Base - nCf2NewBs > 0 
			nCf2Base 	-= nCf2NewBs
			aRet[1]		:= nCf2Base
			aRet[3]		:= Round(nCf2Base * nCf2Aliq / 100,2) 
		Endif 
		//MsgAlert("Produto : " + cCodPrd + " Base: " + cValToChar(aRet[1]) + " Aliquota " + cValToChar(nCf2Aliq) + " Valor " + cValToChar(aRet[3]) )
