...
User Function MLFATA07()
	...
	If !sfSemaforo(.T.,"MLFATA07"+cEmpAnt+cFilAnt+"001","Alguém está remanejando estoque ou enviando pedido exp.")
		Return
	EndIf

	If cEmpAnt+cFilAnt $ "0201"
		If nOpcLoc == 0
			nOpcLoc	:= Aviso("Escolha local de saída!","Para esta filial é necessário escolher entre o armazém 01-Gaspar ou 02-Blumenau",{"01-Gaspar","02-Blumenau"},3)
		Endif
	...
	Endif
	// 1 - Legenda
	Aadd(aHeadBlq		,{"Ok"					,"C9_OK"		   		,"@BMP"     		,1					,0					,""					,				,"C"			,""				,""})

	DbSelectArea("SX3")
	DbSetOrder(2)
	DbSeek("C9_PEDIDO")
	// 2 - Numero pedido
	Aadd(aHeadBlq		,{"Pedido"		,SX3->X3_CAMPO		,SX3->X3_PICTURE	,SX3->X3_TAMANHO	,SX3->X3_DECIMAL	,"",,SX3->X3_TIPO,"",""})
	Private nPxPedido    := Len(aHeadBlq)

	DbSeek("C9_QTDLIB")
	// 3 - Quantidade Liberada
	Aadd(aHeadBlq		,{"Qte Liberada"	,SX3->X3_CAMPO	,SX3->X3_PICTURE	,SX3->X3_TAMANHO	,SX3->X3_DECIMAL	,"",,SX3->X3_TIPO,"",""})
	Private nPxQteLib    := Len(aHeadBlq)

	DbSeek("C9_XNEWQTD")
	// 4 - Nova Quantidade
	Aadd(aHeadBlq		,{"Nova Quantidade"	,SX3->X3_CAMPO		,SX3->X3_PICTURE	,SX3->X3_TAMANHO	,SX3->X3_DECIMAL	,"",,SX3->X3_TIPO,"",""})
	Private nPxNewQte    := Len(aHeadBlq)
	Aadd(aAlter,SX3->X3_CAMPO)

	DbSeek("C9_QTDLIB2")
	// 5 - Saldo Bloqueado
	Aadd(aHeadBlq		,{"Saldo Bloqueado"	,SX3->X3_CAMPO		,SX3->X3_PICTURE	,SX3->X3_TAMANHO	,SX3->X3_DECIMAL	,"",,SX3->X3_TIPO,"",""})
	Private nPxNewSaldo    := Len(aHeadBlq)

	DbSeek("C9_CLIENTE")
	// 6 - Codigo Cliente
	Aadd(aHeadBlq		,{"Cliente"		,SX3->X3_CAMPO		,SX3->X3_PICTURE	,SX3->X3_TAMANHO	,SX3->X3_DECIMAL	,"",,SX3->X3_TIPO,"",""})
	Private nPxCliente    := Len(aHeadBlq)

	DbSeek("C9_LOJA")
	// 7 - Loja
	Aadd(aHeadBlq		,{"Loja"		,SX3->X3_CAMPO		,SX3->X3_PICTURE	,SX3->X3_TAMANHO	,SX3->X3_DECIMAL	,"",,SX3->X3_TIPO,"",""})
	Private nPxLoja    := Len(aHeadBlq)

	DbSeek("A1_NOME")
	// 8 - Nome
	Aadd(aHeadBlq		,{"Razao Social"		,SX3->X3_CAMPO		,SX3->X3_PICTURE	,SX3->X3_TAMANHO	,SX3->X3_DECIMAL	,"",,SX3->X3_TIPO,"",""})
	Private nPxNome    := Len(aHeadBlq)

	DbSeek("A1_MUN")
	// 9 - Cidade
	Aadd(aHeadBlq		,{"Cidade"		,SX3->X3_CAMPO		,SX3->X3_PICTURE	,SX3->X3_TAMANHO	,SX3->X3_DECIMAL	,"",,SX3->X3_TIPO,"",""})
	Private nPxMun    := Len(aHeadBlq)

	DbSeek("C9_ITEM")
	// 10 - Item
	Aadd(aHeadBlq		,{"Item"		,SX3->X3_CAMPO		,SX3->X3_PICTURE	,SX3->X3_TAMANHO	,SX3->X3_DECIMAL	,"",,SX3->X3_TIPO,"",""})
	Private nPxItem   	:= Len(aHeadBlq)

	DbSeek("C9_SEQUEN")
	//11 - Sequencia
	Aadd(aHeadBlq		,{"Seq"		,SX3->X3_CAMPO		,SX3->X3_PICTURE	,SX3->X3_TAMANHO	,SX3->X3_DECIMAL	,"",,SX3->X3_TIPO,"",""})
	Private nPxSequen 	:= Len(aHeadBlq)


	DbSeek("C9_BLEST")
	// 12 - Bloqueio de Estoque
	Aadd(aHeadBlq		,{"Blq.Estoque"		,SX3->X3_CAMPO		,SX3->X3_PICTURE	,SX3->X3_TAMANHO	,SX3->X3_DECIMAL	,"",,SX3->X3_TIPO,"",""})
	...
	sfSemaforo(.F.,"MLFATA07"+cEmpAnt+cFilAnt+"001")	
...
Static Function sfGrava(oInDlg)
	...
	For iX := 1 To Len(oGetBlq:aCols)
		...
		If DbSeek(xFilial("SC9")+oGetBlq:aCols[ix,nPxPedido]+oGetBlq:aCols[ix,nPxItem]+oGetBlq:aCols[ix,nPxSequen])
			...
			If !(SC9->C9_BLEST $ "10") .And. Empty(SC9->C9_SERIENF) .And. Empty(SC9->C9_BLCRED)
				...
				If lExecLib
					...
					If DbSeek(xFilial("SC6")+oGetBlq:aCols[ix,nPxPedido]+oGetBlq:aCols[ix,nPxItem] )     //FILIAL+NUMERO+ITEM
						...
						If nQteLib > 0	// Garante que o Flag de separação vá para o novo item liberado
							MaLibDoFat(SC6->(RecNo()),nQteLib,aLib[1],aLib[2],aLib[3],aLib[4],.F.,.F.,/*aEmpenho*/,/*bBlock*/,/*aEmpPronto*/,/*lTrocaLot*/,/*lOkExpedicao*/,nVlrCred,/*nQtdalib2*/)
						Endif
...
Static Function sfPesquisa()
	...
	If DbSeek(xFilial("SB1")+cVarProd)
		If RegistroOK("SB1",.F.)
			...
			cQry += "       A1_NOME,A1_MUN "
			cQry += "  FROM "+RetSqlName("SC9") + " C9, " + RetSqlName("SA1")+ " A1, " +RetSqlName("SC5") + " C5, " + RetSqlName("SC6") + " C6, " + RetSqlName("SF4") + " F4 "
...
Static Function sfRodape()
	...
	For iX := 1 To Len(oGetBlq:aCols)
		...
		Else
			If iX == oGetBlq:nAt .And. ReadVar() == "M->C9_XNEWQTD"
				nQteLib -= M->C9_XNEWQTD
			Else
				nQteLib -= oGetBlq:aCols[iX,nPxNewQte]
			Endif
		Endif
...
Static Function sfSemaforo(lLock,cKey,cMsg,lTrvEmp,lTrvFil,lExeAuto)

	Local	nTentativas	:= 0
	
	Default lLock		:= .F.
	Default cKey 		:= "MLFATA07"
