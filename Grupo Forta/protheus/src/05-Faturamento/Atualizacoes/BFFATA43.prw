...
User Function BFFATA43(aCodRegBon,cCliente,cLoja,cTabPreco,cCondPg,cFormPg,aRecACQ,cTipoRet)
	...
	#IFDEF TOP
		...
		cQuery += " WHERE ACQ.ACQ_FILIAL='"+xFilial("ACQ")+"' "
		If nRecs > 0
		...
		If Len(aCodRegBon) > 0
			...
			cQuery	+=	"' ')  "
		Endif
		cQuery += "   AND (ACQ.ACQ_CODCLI = '"+cCliente+"' OR ACQ.ACQ_CODCLI='"+Space(Len(ACQ->ACQ_CODCLI))+"')  "
		cQuery += "   AND (ACQ.ACQ_LOJA = '"+cLoja+"' OR ACQ.ACQ_LOJA='"+Space(Len(ACQ->ACQ_LOJA))+"') "
		cQuery += "   AND (ACQ.ACQ_CONDPG = '"+cCondPg+"' OR ACQ.ACQ_CONDPG='"+Space(Len(ACQ->ACQ_CONDPG))+"') "
		cQuery += "   AND (ACQ.ACQ_FORMPG = '"+cFormPg+"' OR ACQ.ACQ_FORMPG='"+Space(Len(ACQ->ACQ_FORMPG))+"') "
		cQuery += "   AND ACQ.D_E_L_E_T_=' ' "
		...
		If FtIsDataOk("ACQ",cCursor) .And. If(ACQ->(FieldPos("ACQ_GRPVEN"))>0, !Empty(FtIsGrpOk((cCursor)->ACQ_GRPVEN,SA1->A1_GRPVEN)),.T.)
			If lValido
				...
				#IFDEF TOP
					...
					cQuery += " WHERE ACR.ACR_FILIAL='"+xFilial("ACR")+"' "
					cQuery += "   AND ACR.ACR_CODREG='"+(cCursor)->ACQ_CODREG+"' "
				...
				If nPrcVend > 0 .And. Posicione("SB1",1,xFilial("SB1")+(cCursor)->ACQ_CODPRO,"B1_MSBLQL") <> "1"
					While ( !Eof() .And. (cCursor)->ACQ_CODREG == (cCursor2)->ACR_CODREG )
						...
						If cTipoRet == "1" 
							...
							Aadd(aRetorno,{"ACR",;											// 1
											(cCursor)->ACQ_CODREG+(cCursor2)->ACR_ITEM,;	// 2
											(cCursor)->ACQ_CODPRO,;							// 3
											(cCursor)->ACQ_DESCRI,;							// 4
											(cCursor2)->ACR_CODPRO ,;						// 5
											SB1->B1_DESC,; 									// 6
											(cCursor2)->ACR_QUANT,;							// 7
											Padr((cCursor2)->ACR_OPER,2),;					// 8
											nPrcVend,;										// 9
											(cCursor2)->ACR_PRCFIX,;						// 10
											cTabPrcFix,;									// 11
											nPrcVend,;										// 12
											(cCursor2)->ACR_LOTE,;							// 13
											(cCursor)->ACQ_XDESMX})							// 14
						Endif
