...
@type function
/*/
User function MLFATC02(nInAlias,cOpcOut)
	...
	ElseIf nInAlias == 6
		...
		ElseIf SC5->C5_LIBEROK=='E' .And. Empty(SC5->C5_BLQ)
			cNomRet	:= "Pedido Encerrado"
		ElseIf !Empty(SC5->C5_LIBEROK) .And. Empty(SC5->C5_BLQ)
			cNomRet	:= "Pedido Liberado"
		
			cQry := "SELECT DISTINCT "
			cQry += "       CASE "
			cQry += "         WHEN C9_BLEST = '  ' AND C9_BLCRED = '  ' AND C9_BLTMS = ' ' THEN 'Liberado' "
			cQry += "         WHEN (C9_BLCRED = '10' AND C9_BLEST = '10' ) OR (C9_BLCRED ='ZZ' AND C9_BLEST = 'ZZ') THEN 'Faturado - Nf:' + C9_NFISCAL "
			...
			cQry += "   AND SC9.C9_FILIAL = '" + Iif(!Empty(xFilial("SC9")) .And. !Empty(xFilial("SC5")),SC5->C5_FILIAL,xFilial("SC9")) + "' "
