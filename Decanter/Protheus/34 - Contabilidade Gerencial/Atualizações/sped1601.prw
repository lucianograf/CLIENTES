...
User Function SPED1601()
    ...
    */
    ...
    cQuery := " SELECT "
    cQuery += "     DISTINCT(A1_COD+A1_LOJA) AS COD_PART_IP"
    cQuery += " FROM "+RetSQLName("SE5")+" SE5, "+RetSQLName("SA6")+" SA6, "+RetSQLName("SA1")+" SA1 "
     cQuery += " WHERE E5_DATA BETWEEN '" + DTOS(dDataDe) + "' AND '" + DTOS(dDataAte) + "' "
    cQuery += "     AND SA1.D_E_L_E_T_ = ' ' "
     cQuery += "     AND SE5.D_E_L_E_T_ = ' ' "
    cQuery += "     AND SA6.D_E_L_E_T_ = ' ' "
    cQuery += "     AND SE5.E5_BANCO = SA6.A6_COD AND A6_CGC <> ''  " 
    cQuery += "     AND SE5.E5_FILORIG = '"+ cFil +"'  "        
    cQuery += "     AND SA1.A1_CGC = SA6.A6_CGC  AND SE5.E5_BANCO = SA1.A1_COD  "        
