...
User Function DECAT003(cPedido as Character,cVend as Character, cProd as Character, nPerDesc as Numeric, nValDesc as Numeric,lSimu as Logical, lFound as Logical) 
   ...
   SZ2->( DbSetOrder(3) )    
    If SZ2->( DBSeek(xFilial("SZ2")+cProd ))
        ...
        If !lFound
                ...
                For nX := 1 to Len(aCamp)
                    ...
                    IF nPerDesc > aCamp[nX][2] 
                        ...
                        If lSimu 
                            lRet := .T.
                        Else 
                            lRet := .T. // Estava dentro do if, mas quando já estava bloqueado (cópia de um pedido ele não deixava true novamente)
                            IF !(SC5->C5_XBLQCMP)
                                RecLock("SC5",.F.) 
                                    SC5->C5_BLQ := "1"
                                    SC5->C5_LIBEROK := " "
            ...
            EndIf 
        Else
            //Verifica se encontrou a campanha.
           If  Len(aCamp)>0
                lRet   := .T.
            Else    
                lRet   := .F.
