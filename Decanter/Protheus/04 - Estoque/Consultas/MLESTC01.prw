...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
Static Function fAjusta(cOpcao,cLoteCtl,cDoc,dData,nQuant,cEndereco,cNumSeq,cTipoMov,cNumLote,cSerie,cCliFor,cLoja,dDtValid,cFolder,nQuant2UM)
	//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	Local aArea  := GetArea()
	...
	Do Case
		...
		IF DBSeek(xFilial("SB2")+cProduto+cLocal)
			RecLock("SB2",.F.)
			SB2->B2_QATU    := nQuant
			SB2->B2_QTSEGUM := nQuant2UM //ConvUM(cProduto,nQuant,0,2) // 2UM
			MsUnlock()
			nSB2 := SB2->B2_QATU
		Endif
		...
		IF DBSeek(xFilial("SB9")+cProduto+cLocal+DTOS(dUltFech))
			RecLock("SB9",.F.)
			SB9->B9_QINI    := nQuant
			SB9->B9_QISEGUM := nQuant2UM //ConvUM(cProduto,nQuant,0,2) // 2UM
			MsUnlock()
			nSB9 := SB9->B9_QINI
		Endif
		...
		IF !Empty(aDetDASB8[oDetDASB8:nAT][12])
			...
			IF !Eof()
				...
				SB8->B8_QACLAS2 := nQuant2UM //ConvUM(cProduto,nQuant,0,2) // 2UM
				MsUnlock()
			Endif
		Endif
		...
		IF !Empty(aDetDASDA[oDetDASDA:nAT][13])
			...
			IF !Eof()
				RecLock("SDA",.F.)
				SDA->DA_SALDO   := nQuant
				SDA->DA_QTSEGUM := nQuant2UM // ConvUM(cProduto,nQuant,0,2) // 2UM
				MsUnlock()
			Endif
		Endif
		...
		IF !Empty(aDetSD5[oDetSD5:nAT][14])
			...
			IF !Eof()
				RecLock("SD5",.F.)
				SD5->D5_QUANT   := nQuant
				SD5->D5_QTSEGUM := nQuant2UM //ConvUM(cProduto,nQuant,0,2) // 2UM
				MsUnlock()
			Endif
		Endif
		...
		IF !Empty(aDetSBF[oDetSBF:nAT][11])
			...
			IF !Eof()
				RecLock("SBF",.F.)
				SBF->BF_QUANT   := nQuant
				SBF->BF_QTSEGUM := nQuant2UM //ConvUM(cProduto,nQuant,0,2) // 2UM
				MsUnlock()
				If SBF->BF_QUANT <= 0
					DeletSBF()
				Endif
		...
		IF !Empty(aDetSDB[oDetSDB:nAT][14])
			...
			IF !Eof()
				RecLock("SDB",.F.)
				SDB->DB_QUANT   := nQuant
				SDB->DB_QTSEGUM := nQuant2UM //ConvUM(cProduto,nQuant,0,2) // 2UM
				MsUnlock()
			Endif
		Endif
...
Static Function fSaldoAtual()
	//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	U_GravaSX1("MTA300","01",cLocal)
	U_GravaSX1("MTA300","02",cLocal)
	U_GravaSX1("MTA300","03",cProduto)
	U_GravaSX1("MTA300","04",cProduto)
