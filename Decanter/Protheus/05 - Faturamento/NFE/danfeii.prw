...
#DEFINE MAXITEMC   050  //DECANTER (original era 035)                 // Máxima de caracteres por linha de produtos/serviços
...
#DEFINE MAXVALORC  012  //DECANTER (original era 009)                 // Máximo de caracteres por linha de valores numéricos
#DEFINE MAXCODPRD  045  //DECANTER (original era 050)                 // Máximo de caracteres do codigo de produtos/servicos
...
private oRetNF
//INICIO CUSTOMIZADO DECANTER
//TSCB57 - William Farias - 28/08/2019 - INICIO
Private cDecImpAut := ""
Private cDecLocPdf := ""
//TSCB57 - FIM
//FIM CUSTOMIZADO DECANTER
...
If nTipo <> 1
	...
	If lAutomato .or. oSetup:GetProperty(PD_PRINTTYPE) == IMP_PDF
		...
		//INICIO CUSTOMIZADO DECANTER
		//TSCB57 - William Farias - 28/08/2019 - INICIO
		//Grava impressora escolhida pelo usuario para imprimir o boleto automaticamente.
		cDecImpAut := "IMP_PDF"
		cDecLocPdf := oDanfe:cPathPDF
		//TSCB57 - FIM
		//FIM CUSTOMIZADO DECANTER
	ElseIf oSetup:GetProperty(PD_PRINTTYPE) == IMP_SPOOL
		...
		//TSCB57 - William Farias - 28/08/2019 - INICIO
		//Grava impressora escolhida pelo usuario para imprimir o boleto automaticamente.
		cDecImpAut := oDanfe:cPrinter
		//TSCB57 - FIM
		//FIM CUSTOMIZADO DECANTER
	EndIf

	If lIsLoja .And. !FwIsInCallStack('U_UNIM007')
		MV_PAR01 := SF2->F2_DOC
		MV_PAR02 := SF2->F2_DOC
	...
	//INICIO CUSTOMIZADO DECANTER
	//TSCB57 - William Farias - 30/08/2019 - INICIO
	ElseIf FwIsInCallStack('U_UNIM007')
		MV_PAR01 := SF2->F2_DOC
		MV_PAR02 := SF2->F2_DOC
		MV_PAR03 := SF2->F2_SERIE
		MV_PAR04 := 2	//[Operacao] NF de Saida
				...
				If !Empty(aXML[nX][2])
					...
					If (!Empty(cAutoriza) .Or. !Empty(cCodAutDPEC) .Or. Alltrim(aXML[nX][8]) $ "2,5") .And. !cCodRetNFE $ RetCodDene()
						...
						If Empty(cAviso) .And. Empty(cErro)
							...
							//INICIO CUSTOMIZADO DECANTER
							//TSCB57 - William Farias - 28/08/2019 - INICIO
							//Chama impressao de boleto ao final da impressao
							If aNotas[nX][2] == "S" //Saída oSetup:aOptions[PD_VALUETYPE] 
								//Em 22/01/2020 - Adicionado verificacao de quando reimpresso via rotina da DANFE padrao.
								If FWIsInCallStack("U_UNIM007") .or.  FWIsInCallStack("U_PrtNfU")
									_oPrtDanf:= oDanfe
									U_DECA152(aNotas[nX])
								EndIf
						EndIf
...
If nTipo <> 1
	...
	PRIVATE OFONT12N   := TFontEx():New(oDanfe,"Times New Roman",10,10,.T.,.T.,.F.)// 12 //DECANTER - Alterado 09/06/20 por TSCB87 - Leandro  Inocencio - Anterior 11
...
//DECANTER - Início Customização
//09/06/20 - TSCB87 - Leandro Inocencio
//Inclusão da transportadora de Redespacho nas Informações Complementares

If !EMPTY(ALLTRIM(SF2->F2_REDESP))

	dbSelectArea("SA4")
	dbSetOrder(1)
	IF MsSeek(xFilial("SA4")+ALLTRIM(SF2->F2_REDESP))

		cAux := "MERCADORIA SERÁ REDESPACHADA PELA TRANSPORTADORA: "
		cAux += ALLTRIM(SA4->A4_NOME)
		
		If !Empty(ALLTRIM(SA4->A4_CGC))
			cAux += " - CNPJ: "+Transform(ALLTRIM(SA4->A4_CGC),x3Picture("A4_CGC"))
		EndIf
		
		If !Empty(ALLTRIM(SA4->A4_INSEST))
			cAux += " - IE: "+ALLTRIM(SA4->A4_INSEST)
		EndIf		
		
		cAux += " End: "+ALLTRIM(SA4->A4_END)
		cAux += " Bairro: "+ALLTRIM(SA4->A4_BAIRRO)
		cAux += " Cidade: "+ALLTRIM(SA4->A4_MUN)+"/"+ALLTRIM(SA4->A4_EST)
		cAux += " Cep: "+Transform(ALLTRIM(SA4->A4_CEP),x3Picture("A4_CEP"))
...
nForTo		:=	Len(cStrAux)/29 //DECANTER ANTERIOR 25 - TSCB87 - Leandro Inocencio 09/06/20
nForTo		:=	Iif(nForTo>Round(nForTo,0),Round(nForTo,0)+1,Round(nForTo,0))
For nX := 1 To nForTo
	oDanfe:Say(nLinCalc,096,SubStr(cStrAux,Iif(nX==1,1,((nX-1)*29)+1),29), oFont12N:oFont )//DECANTER - Alterado limitador linha SUBSTR anterior 25 TSCB87 - Leandro Inocencio 09/06/20
	nLinCalc+=10
Next nX
...
For nY := 1 To nLenItens
	...
	Else
		...
		nAuxH += aTamCol[1]
		oDanfe:Say(nLinha, nAuxH + 2, aAux[1][2][nY], oFont07:oFont) // DESCRICAO DO PRODUTO //27/12/2019 - TSC852 - Willian Doerner: Alteracao de Tamanho da Fonte
		nAuxH += aTamCol[2]
