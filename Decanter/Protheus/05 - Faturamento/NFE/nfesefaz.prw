...
// Customizado Decanter
	Local nICVAnt 		:= 0 	//Calculo para mensagem dados adicionais icms st - decanter - TSCB18 Luis França
	Local nIFCAnt 		:= 0	//Cálculo para mensagem dados adicionar do FECP ST - Decanter
	Local nICDAnt 		:= 0	//Cálculo para mensagem dados adicionar do ICMS ST sem FECP - Decanter
	Local nICBAnt 		:= 0 	//Calculo para mensagem dados adicionais icms st - decanter - TSCB18 Luis França
...
//Customizado - Variaveis DECANTER - TSCB18 Luis França
	Private cMensIcmSt  := ''
// 26/11/2020 - Marcelo Alberto Lauschner - Decanter - Informar e-mail de transportadora de redespacho no xml -
	Private cMailRedesp	:= ''
// 10/06/2021 - Marcelo Alberto Lauschner - Decanter - Data de Saída/Entrega informada no pedido de venda.
	Private dDatSaiEnt	:= CTOD("")
			...
			If MsSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
				...
				ElseIf FunName() == "SPEDNFSE"
					...
					While !Eof() .And. xFilial("SF3") == SF3->F3_FILIAL .And.;
						...
						// Customização Decanter - 10/06/2021 - VErifica se tem data de saída/entrega
						If SC5->(FieldPos("C5_ZSAIENT")) > 0 .And. !Empty(SC5->C5_ZSAIENT) .And. SC5->C5_ZSAIENT <> dDatSaiEnt
							dDatSaiEnt	:= SC5->C5_ZSAIENT
						Endif
				...
				Else
					...
					*/
					...
					If !Empty(SF2->F2_TRANSP)
						...
						//Customização Decanter - 26/11/2020 - Marcelo Alberto Lauschner
						cMailRedesp	:= IIf(!Empty(SA4->A4_EMAIL ),Alltrim(SA4->A4_EMAIL),"")
						...
						If !Empty(SF2->F2_REDESP)
							...
							If !Empty(SA4->A4_EMAIL)
								If !Empty(cMailRedesp)
									cMailRedesp	+= ";"
								Endif
								cMailRedesp += Alltrim(SA4->A4_EMAIL)
							Endif
							...
							MsSeek(xFilial("SA4")+SF2->F2_TRANSP)
						Endif
						aadd(aTransp,cMailRedesp)
						// Fim Customização email para transportadora e redespacho
					...
					If lContinua
						...
						If MsSeek(xFilial("SB5")+(cAliasSD2)->D2_COD)
							If SB5->(FieldPos("B5_DESCNFE")) > 0 .And. !Empty(SB5->B5_DESCNFE)
								//especifico decanter cbenef
								IF (cAliasSD2)->D2_CF = '5152' .AND. SUBSTR((cAliasSD2)->D2_CLASFIS,2,2) ='51'
									cInfAdic := 'SC830073'
								elseif SUBSTR((cAliasSD2)->D2_CF,1,1) = '5' .AND. (cAliasSD2)->D2_PICM = 25 .AND. SUBSTR((cAliasSD2)->D2_CLASFIS,2,2) ='51'
									cInfAdic := 'SC830018'
								ELSE
								cInfAdic := ""
								ENDIF
						...
						// Customização Decanter - 28/10/2021 - Destacar ordem de compra informada no pedido nas observações da nota
						If (SC5->(FieldPos("C5_ZXPED")) > 0) .And. !Empty(SC5->C5_ZXPED) .And. !Alltrim("Ordem Compra: "+SC5->C5_ZXPED) $ cMensCli
							cMensCli	+= Alltrim("Ordem Compra: "+SC5->C5_ZXPED)
						Endif
						...
						*/
								...
								If Empty(aPedido)
									// Customização Decanter - Verifica preenchimento de informação para levar para o XML - Ordem de compra
									If (SC5->(FieldPos("C5_ZXPED")) > 0) .And. !Empty(SC5->C5_ZXPED)
										aPedido := {Iif(SC5->(FieldPos("C5_NTEMPEN")) > 0,Alltrim(SC5->C5_NTEMPEN),""),AllTrim(SC5->C5_ZXPED),""}
									Else
								...
								If SC6->(FieldPos("C6_NUMPCOM")) > 0 .And. SC6->(FieldPos("C6_ITEMPC")) > 0
									...
									If !Empty(SC6->C6_NUMPCOM) .And. !Empty(SC6->C6_ITEMPC)
										aadd(aPedCom,{SC6->C6_NUMPCOM,SC6->C6_ITEMPC})
										//+-----------------------------------------------------------------------------------
										// Customização Decanter
										// 28/10/2021 - Adicionada opção para levar Ordem de Compra customizada
									ElseIf (SC5->(FieldPos("C5_ZXPED")) > 0) .And. !Empty(SC5->C5_ZXPED)
										aadd(aPedCom,{Substr(SC5->C5_ZXPED,1,15),SC6->C6_ITEM})
										// +----------------------------------------------------------------------------------
									Else
										aadd(aPedCom,{})
									EndIf
								...
								// Customização Decanter - 26/08/2020 - Somente descrição do Produto
						...
						*/
								...
								// 05/05/2021 - Customização Decanter - Verifica se o pedido deve ser gerar os itens na Segunda Unidade de medida
								If (SC5->(FieldPos("C5_ZUM2NFE")) > 0 .And. SC5->C5_ZUM2NFE == "2" )
									aadd(aProd,	{Len(aProd)+1,;
										cCodProd,;
										Iif(SB5->(ColumnPos("B5_Z2CODBR")) > 0,IIf(Val(SB5->B5_Z2CODBR)==0,"",StrZero(Val(SB5->B5_Z2CODBR),Len(Alltrim(SB5->B5_Z2CODBR)),0)) ,IIf(SB5->(ColumnPos("B5_2CODBAR")) > 0,IIf(Val(SB5->B5_2CODBAR)==0,"",StrZero(Val(SB5->B5_2CODBAR),Len(Alltrim(SB5->B5_2CODBAR)),0)),"")),;//aprod[45]  Código de barra da segunda unidade de medida.
									cDescProd,;
									...
									SB1->B1_EX_NCM,;
										cD2Cfop,;
										SB1->B1_SEGUM ,;                 //retUn2UM( lNoImp2UM, cCFOPExp, Alltrim((cAliasSD2)->D2_CF), SB5->B5_UMDIPI, SB1->B1_UM ),;
										ROUND(((cAliasSD2)->D2_QUANT/SB1->B1_CONV),2) ,;    //retQtd2UM( lNoImp2UM, cCFOPExp, Alltrim((cAliasSD2)->D2_CF), SB5->B5_CONVDIP, (cAliasSD2)->D2_QUANT ),;
						...
						*/
					    ...
					    If SFT->(DbSeek(xFilial("SFT")+"S"+(cAliasSD2)->(D2_SERIE+D2_DOC+D2_CLIENTE+D2_LOJA+PadR(D2_ITEM,TamSx3("FT_ITEM")[1])+D2_COD)))
							//DECANTER CUSTOMIZADO
							//Calculo para mensagem dados adicionais icms st - decanter - TSCB18 Luis França
						   	If Right(SFT->FT_CLASFIS,2)== '00' .and. (SFT->FT_ICMSRET > 0) ;
						   	  .And. !(AllTrim(SFT->FT_CFOP) == '6108' .or. AllTrim(SFT->FT_CFOP) == '6910')

								nICBAnt := nICBAnt + Round(SFT->FT_BASERET,2)
								nICVAnt := nICVAnt + Round(SFT->FT_ICMSRET,2)
								nIFCAnt += Round(SFT->FT_VFECPST ,2)
								nICDAnt += Round(SFT->FT_ICMSRET - FT_VFECPST ,2)
								cMensIcmSt := " BC ICMS ANTEC.: " + cValToChar(Round(SFT->FT_BASERET,2)) +" ICMS ANTEC.: " + cValToChar(Round(SFT->FT_ICMSRET,2))
								aProd[Len(aProd),25] += cMensIcmSt
						...
						*/
						...
						If !Empty(aDest[15])
							...
							EndIf
							// Customização Decanter
							//Tratamento para incluir a mensagem em informacoes adicionais ICMS ST - Decanter - TSCB18 - Luis França
							If !Empty(nICVAnt)
								//nIFCAnt += Round(SFT->FT_VFECPST ,2)
								//nICDAnt += Round(SFT->FT_ICMSRET - FT_VFECPST ,2)
								cMensIcmSt := "Inf. Fisco BASE CALCULO ICMS ANTECIPADO: R$ " + Alltrim(Transform(nICBAnt,"@E 999,999,999.99") )
								cMensIcmSt += " VALOR ICMS ANTECIPADO: R$ " + Alltrim(Transform(nICVAnt,"@E 999,999,999.99"))
								cMensIcmSt += " (FECP-ST R$:"+ Alltrim(Transform(nIFCAnt,"@E 999,999,999.99"))
								cMensIcmSt += " / ICMS-ST R$ "+Alltrim(Transform(nICDAnt,"@E 999,999,999.99"))
								cMensIcmSt += ") INCIDENCIA DO IPI CFE. MP 690/2015, LEI 13.241/2015 E DECRETO 8.512/2015."
								//MsgAlert(cMensIcmst)
								cMensFis += cMensIcmSt
							EndIF
						EndIF
		...
		Else
			...
			if (nPosCpoEsp := SF1->(ColumnPos("F1_ESPECI"+cScan))) > 0
				...
				While ( !Empty(cScan) )
					...
					//(+-----------------------------------------------------------------------------------------
					// Customização Decanter
					// Corrige falha do não preenchimento do campo Especie e adiciona o peso bruto e liquido a nota fiscal
					ElseIF Empty(cEspecie) .And. cScan == "1"
						aadd(aEspVol,{ "CAIXAS", FieldGet(FieldPos("F1_VOLUME"+cScan)) , SF1->F1_PLIQUI , SF1->F1_PBRUTO})
					//)+---	
					EndIf
...
//Geracao do arquivo XML
							If !Empty(aNota)
								...
								EndIf
								//customizado decanter
								if nDiferido > 0 .AND. SF4->F4_FILIAL = '0101' .AND. SF4->F4_PICMDIF>0 .AND. SF4->F4_PICMDIF<100
									cMensCpl += "Diferimento parcial do ICMS Cfe. Anexo 2 Art. 10 do RICMS/SC. Valor do diferimento: R$ " + ConvType(nDiferido,15,2) + "."
								EndIf

								If SF4->F4_PICMDIF>0 .AND. SF4->F4_PICMDIF<100
									If nValDifer > 0
										//Comentei 13/12/2022, solicitado pelo Dep. Fiscal
										cMensCpl += "Diferimento parcial do ICMS Cfe. Livro III, Art. 1ºK do RICMS/RS do decreto 55797/2021. Valor do diferimento: R$ " + ConvType(nValDifer,15,2) + "."
										//Alterada mensagem Andre 04/03/2022  cMensCpl += "Diferimento do ICMS que exceder 12% - Base Legal Item II da Subseção III da Seção IV do Apêndice II do RICMS/RS. Valor do ICMS Diferido R$ " + ConvType(nValDifer,15,2) + "."
									EndIf
								ENDIF
								//fim decanter
								If nValDifer > 0
...
// Customização Decanter - 10/06/2021 - Verifica se tem Data de Entrega que veio do Pedido de Venda
	If lDSaiEnt .And. Empty(dDatSaiEnt)
		cDSaiEnt	:=  ""
	Else
		If !Empty(dDatSaiEnt)
			cDSaiEnt 	:= 	ConvType(dDatSaiEnt)
		Else
			cDSaiEnt 	:=  ConvType(aNota[03])
		Endif
		cDSaiEnt += "T"
		If Len(AllTrim(aNota[06])) > 5
			cDSaiEnt +=	ConvType(aNota[06])
		Else
			cDSaiEnt +=	ConvType(aNota[06])+":00"
		Endif
	Endif
//cString += NfeTag('<dhSaiEnt>', cDSaiEnt)
...
// Customização Decanter - Alterar código de barras
	If aNota[4] == "1"// Tipo de nota - 1-Saída

		DbSelectArea("SD2")
		DbSetOrder(3) // D2_FILIAL+D2_DOC+D2_SERIE+D2_CLIENTE+D2_LOJA+D2_COD+D2_ITEM
		dbseek(xFilial("SD2")+aNota[2]+aNota[1])

		DbSelectArea("SC5")
		DbSetOrder(1)
		DbSeek(xFilial("SC5")+aProd[38] )
		If (SC5->(FieldPos("C5_ZUM2NFE")) > 0 .And. SC5->C5_ZUM2NFE == "2" )
			cEan		:= aProd[03]
...
//+--------------------------------------------------------------------------------------------//
			If !Empty(SF1->F1_HAWB)
				...
				If ValType(aRetImp) == "A"
					For nX := 1 To Len(aRetImp)
						cMsgaFill += aRetImp[nX]
					Next
				EndIf
			EndIf

			cString += ConvType(cMsgaFill,Len(cMsgaFill))+" "
//+--------------------------------------------------------------------------------------------//
//| ----------------------------------------- aFill -------------------------------------------//
//+--------------------------------------------------------------------------------------------//
//
