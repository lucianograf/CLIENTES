...
if !Empty(aNota)
    if Len(aProd) > 0
        ...
        cString	+= servicosNac( aProd, cNatOper, lNFeDesc, cDiscrNFSe,aRetSF3)
...
static function prestNac()
    ...
    //Local	cIM		        := if( type( "oSigamatX" ) == "U",SM0->M0_INSCM,oSigamatX:M0_INSCM )
...
Static Function tomadorNac( aDest, cCodMun )
    ...
    If !lSemTomador
        ...
        if( allTrim( aDest[9] ) == "EX")
			...
			cString	+= '<xCidade>.</xCidade>'
			cString	+= '<xEstProvReg>PROVINCIA</xEstProvReg>'
			cString	+= '</endExt>'
        Else
            cString	+= "<endNac>"
...
/*/
//-----------------------------------------------------------------------
static function servicosNac( aProd, cNatOper, lNFeDesc, cDiscrNFSe,aRetSF3)	
    ...
    cString += '</locPrest>'
    cString += '<cServ>'
    cString += IIF( !EMPTY( allTrim( aProd[1][32]) ) ,'<cTribNac>'+ allTrim( aProd[1][32]) +'</cTribNac>', "" )
...
static function valoresNac( aISSQN, aRetido, aTotal, aDest, cCodMun,aLeiTrp,lRecIrrf, aProd )
    ...
    nValTotFed := aPISXML[1] + aCOFINSXml[1] // total dos impostos federais
    ...
    do case
        ...
        case !Empty( aDest[9] ) .AND. aDest[9] $ "EX"                   //EX     
    ...
    cString += '<pTotTribFed>'+  ALLTRIM( STR(nValTotFed,5,2) ) +'</pTotTribFed>
    cString += '<pTotTribEst>0.00</pTotTribEst>
    cString += '<pTotTribMun>0.00</pTotTribMun>
