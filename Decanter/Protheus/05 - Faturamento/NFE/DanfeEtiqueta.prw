...
user function ImpDfEtq(cUrl, cIdEnt, lUsaColab)
    ...
    // 22/12/2022 - Customização Decanter 
    Private cNumPed     := ""
        ...
        if (!empty(cProtocolo) .or. !empty(cDpecProt))
            ...
            if nTipoDoc == 2
                ...
                if lSeek 
                 ...
                 // 22/12/2022 - Customização Decanter  - Pega número do pedido de venda 
                DbSelectArea("SD2")
...
static function DanfeSimp(oPrinter, nPosY, nPosX, oFontTit, oFontInf, aEmit, aNfe, aDest)
    ...
    If !Empty(cNumPed)
        oPrinter:Say( 65 + nPosY, 170 + nPosX, "Pedido:"+cNumPed, oFontTit )
    Endif 
    ...
    /* Customização Decanter - Alteração descrição Remetente 
    ...
    If len(aEmit[1]) > 35
        oPrinter:Say( 145 + nPosY, 80 + nPosX, SubStr(aEmit[1],1,35), oFontTit)
        oPrinter:Say( 153 + nPosY, 80 + nPosX, SubStr(aEmit[1],36,35), oFontTit)
    Else
        oPrinter:Say( 145 + nPosY, 80 + nPosX, aEmit[1], oFontTit)
    Endif
    ...
    // Customização Decanter - Alteração de descriçaõ do Destinataário 
   /* oPrinter:Say( 228 + nPosY, 35 + nPosX, cTitNome, oFontTit)
    ...
    If Len(aDest[1]) > 50
        oPrinter:Say( 228 + nPosY, 35 + nPosX, SubStr(aDest[1], 1 ,50), oFontTit)
        oPrinter:Say( 236 + nPosY, 35 + nPosX, SubStr(aDest[1], 51,50), oFontTit)
    Else
        oPrinter:Say( 228 + nPosY, 35 + nPosX, aDest[1], oFontTit)
    Endif
...
static function impZebra(aNFe, aEmit, aDest)
    ...
    // 22/12/2022 - Customização Decanter - Corrigido para fazer Alltrim() para comparar com tamanho 14
    lEmitJurid := Len( Alltrim(aEmit[nCNPJ]) ) == 14 //Se emitente pessoa jurídica lEmitJurid = .T.
    lDestJurid := Len( Alltrim(aDest[nCNPJ]) ) == 14 //Se destinatário pessoa jurídica lDestJurid = .T.
    ...
    If !Empty(cNumPed)
        MSCBSay(50, 15 , "Pedido:"+cNumPed , "N", "A", cFontMaior)
    Endif 
    ...
    // 22/12/2022 - Marilu 
    // Customização Decanter - Não imprimir o título do campo - Só imprimir direto a Razão Social 
    ...
    // Customização Decanter - Desativa título 
   ...
   /* if lNomeEmit
    ...
    endIf*/
    // 22/12/2022 -Customização Decanter - Não imprimir o título do campo - Só imprimir direto a Razão Social 
    MSCBSay(04, 60, AllTrim( aEmit[nNome] ), "N", "A", cFontMaior)
    ...
    /* Customização Decanter - Retira a quebra de linha e informa em letra maior o nome do cliente 
    if lNomeDest
    ...
    MSCBSay(04, 113.75, allTrim( aDest[nNome] ), "N", "A", cFontMaior)
