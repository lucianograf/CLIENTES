		...
		If (lTMSCTe) .Or. (AModNot(SF2->F2_ESPECIE)=="57") .Or. (FunName()$"TMSA200" .Or. FunName()$"TMSA500")
			...
			If (!(cAliasDT6)->(Eof())) // (01)
				...
				While !(cAliasDT6)->(Eof()) // (02)
					...
					cNatOper := SubStr(AllTrim(FWGetSX5("13",cCFOP)),1,55)
					...
					If FindFunction("TmsPsqDY4") .And. TmsPsqDY4( (cAliasDT6)->DT6_FILDOC, (cAliasDT6)->DT6_DOC, (cAliasDT6)->DT6_SERIE )							
						...
						cQuery += "	     DTC.DTC_NFEID, DTC.DTC_CLIDES, DTC.DTC_LOJDES, DTC.DTC_SQEDES, DTC.DTC_TIPNFC, " + CRLF
					...
					//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
					If lDT6SqeDes .And. (cAliasDT6)->DT6_DOCTMS $ '67' //Devolução ou Reentrega utilizando sequencia de endereço
						cSeqEntr := (cAliasDT6)->DT6_SQEDES
					EndIf
					DUL->(DbSetOrder(2))
					If !Empty(cSeqEntr) .And. DUL->(MsSeek(xFilial('DUL')+(cAliasDT6)->DT6_CLIDES + (cAliasDT6)->DT6_LOJDES + cSeqEntr))
					...
					While !(cAliasTagJ)->(Eof())
						...
						ElseIf SubStr((cAliasTagJ)->FT_CLASFIS, 2, 2) == "60"
							...
							cString += '<vICMSSTRet>'+ ConvType(nVALICM + nCREDPRES , 15, 2) + '</vICMSSTRet>'
							cString += '<pICMSSTRet>'+ ConvType(nALIQICM,  6, 2) + '</pICMSSTRet>'
					...
					If	!AllTrim((cAliasDT6)->DT6_DOCTMS) == StrZero( 8, Len(DT6->DT6_DOCTMS)) .And.;
						...
						cString += '<xOutCat>' + NoAcentoCte(FWGetSX5("DU",(cAliasAll)->DB0_TIPCAR) ) + '</xOutCat>'
						(cAliasAll)->(DbCloseArea())
...
Static Function fPergunte(cGrupo, lVar)
