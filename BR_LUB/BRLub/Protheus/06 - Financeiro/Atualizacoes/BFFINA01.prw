				...
				While !Eof()
					...
					Else
						If DataValida(STOD(aRetSE2[2])) < (cNextAlias)->E2_VENCREA 
							If MsgYesNo("Data de Vencimento Real no Título (" + DTOC((cNextAlias)->E2_VENCREA )+") está maior que a data do Vencimento do Código de Barras (" + DTOC(STOD(aRetSE2[2]))+")!" +Chr(13)+Chr(10)+" Deseja lançar ? ")
								lAddSe2	:= .T. 
							Endif
						Else
							lAddSe2	:= .T. 
