...
//(+---------------------------------
// Customização Atrialub - Alterado o tamanho de 038 para 045
...
If nTipo <> 1
	...
	Else
		...
		Endif
		//(+----------------------------------------------------------------------------------------------
		// Customização Atrialub - Monta as perguntas automaticamente se a chamada vier de outra rotina externa
		//lPergunte := Pergunte("NFSIGW",.T.)
		If IsInCallStack("U_BFFATA16")
			lPergunte := .T.
		elseif IsInCallStack( 'U_BFCOMA11' )
			lPergunte := .T.
		elseif IsInCallStack( 'U_MT103FIM' )
			lPergunte := .T.
		ElseIf lVerPerg
			lPergunte := Pergunte("NFSIGW",.T.)
		EndIf
...
If !lImpDir
	...
	#IFDEF TOP
		...
		// Para filtrar apenas notas selecionadas na rotina de impressão de Notas/Boletos do faturamento
		If IsInCallStack("U_BFFATA16") .And. Type("aRecSF2") == "A" .And. Len(aRecSF2) > 0
			cWhere += " AND F3_NFISCAL IN("

			For iP := 1 To Len(aRecSF2)
				If iP > 1 
					cWhere += ","
				Endif
				cWhere += "'"+aRecSF2[iP]+"'"				 
			...
			cWhere += ") "
		Else 	
			// Se houver um range maior que 20 , força a impressão de 1 nota só 
			If Val(MV_PAR02) -  Val(MV_PAR01) > 20
				MV_PAR01 	:= MV_PAR02
				MsgAlert("O intervalo de impressão de notas não pode passar de 20 notas por vez!","Danfeii")
			Endif 
					...
					If (!Empty(cAutoriza) .Or. !Empty(cCodAutDPEC) .Or. Alltrim(aXML[nX][8]) $ "2,5") .And. !cCodRetNFE $ RetCodDene()
						...
						Else
							...
							If MsSeek(xFilial("SF2")+aNotas[nX][05]+aNotas[nX][04]+aNotas[nX][06]+aNotas[nX][07]) .And. Alltrim(aXML[nX][8])$"1,3,4,6,7" .Or. ( Alltrim(aXML[nX][8]) $ "2,5"  .And. !Empty(cAutoriza) )
								...
								//(+-------------------------------------------------------------------------------------------------------------
								// Customização Atrialub
								// Somente usuarios do faturamento tem permissao para atualizar flag de impressao do Danfe					
								If __cUserId $ GetNewPar("GM_USRPTDF",__cUserId)
								//)+-------------------------------------------------------------------------------------------------------------
									...
									//(+-------------------------------------------------------------------------------------------------------------
									// Customização Atrialub
									// Grava log de impressão do Danfe 
									U_GMCFGM01("IN",,"Impressão da Danfe com canhoto "+SF2->F2_SERIE+"/"+SF2->F2_DOC ,FunName())
									
									// 24/08/2019 - Específico para Redelog x Iconic - Enviar o XML por e-mail 
									If cEmpAnt == "06"
										// Desativado em 19/08/2021
										//If SF2->F2_CLIENTE $ "000013"
										//	U_BFFATW24(SF2->F2_DOC)
										//Endif
									Endif	
						...
						If Empty(cAviso) .And. Empty(cErro)
							...
							// | JEAN SAGGIN 	| Chamada da rotina de impressão de boletos
							if ExistBlock( 'BOLRULES' )
								U_BOLRULES( 'SF2', @oDanfe )
							EndIf
							/* | JEAN SAGGIN | Chamada da impressão de boletos e duplicatas durante impressão dos danfes | FIM */
...
Local	cProdAgl	:= ""
Local	cProdLote	:= ""
Local	lAgltPrdLt	:= cEmpAnt+cFilAnt $ "0501"
Local	aItensAglt	:= {}
Local	nLastLin	:= 0
Local	cMsgAuxDf	:= ""
local   cFoneCont   := AllTrim( SuperGetMv( 'MV_X_TELEF',,'0800 000 0688' ) )		// Fone para contato
local	cStrAux     := "" as character
local   cImpresso   := "" as character
local	aStrAux 	:= {} as array
local	nTamNome	:= 30 as numeric
	...
	For nX := 1 To 3
		...
		For nY := 1 To Min(9, nFaturas)
			Do Case
				Case nX == 1
					If nFaturas > 1
						AAdd(aAux, AllTrim(oFatura:_Dup[nY]:_nDup:TEXT)+Iif(Type("oFatura:_fat:_nFat") <> "U", "/"+AllTrim(oFatura:_fat:_nFat:TEXT),""))			
					Else
						AAdd(aAux, AllTrim(oFatura:_Dup:_nDup:TEXT)+Iif(Type("oFatura:_fat:_nFat") <> "U", "/"+AllTrim(oFatura:_fat:_nFat:TEXT),""))
					EndIf
				Case nX == 2
					If nFaturas > 1
...
EndIf
	...
	//(+-------------------------------------------------------------------------------------------------
	// Customização Atrialub
	// Atualiza flag de bloqueio de pedido
	// Altera portador títulos em carteira quando não for boleto
	If(MV_PAR04==2) 
		...
		If !Empty(SD2->D2_PEDIDO) .And. __cUserId $ GetNewPar("GM_USRPTDF",__cUserId)
			...
			If SC5->C5_BANCO $ "887#988#993#985#987#900#991#992"
				DbSelectArea ("SA6")
				Dbsetorder(1)
				...
				SE1->E1_NUM == SF2->F2_DUPL .And. SE1->E1_CLIENTE == SF2->F2_CLIENTE .And. SE1->E1_LOJA == SF2->F2_LOJA
					// Somente se o título ainda não tiver um portador e estiver em carteira. 
					// Corrigido em 07/07/2019 - Chamado 23.441 
					If Empty(E1_PORTADO) .And. SE1->E1_SITUACA == "0"
						RecLock("SE1",.F.)
				...
				EndDo
			Endif
			//If Empty(aTransp[11]) // Sempre assume o valor do campo F2_VOLUME1 
			aTransp[11] := Transform(SF2->F2_VOLUME1,"@R 99,999")
			//Endif
			If Empty(aTransp[12])
				aTransp[12] := "DIV"
			Endif
			If Empty(aTransp[13])
				aTransp[13] := SF2->F2_ESPECI2	//SC5->C5_BOX
			Endif
			If Empty(aTransp[14])
				aTransp[14] := SC5->C5_BANCO +" / "+ SC5->C5_NUM
			Endif
			If Empty(aTransp[15])
				aTransp[15] := Transform(SF2->F2_PBRUTO , "@E 999,999.99")
			Endif
			If Empty(aTransp[16])
				aTransp[16] := Transform(SF2->F2_PLIQUI , "@E 999,999.99")
			Endif
		Endif
		// 26/03/2024 - Imprimir no Danfe as mensagens das retenções 
		If Type("oTotal:_retTrib:_vRetPIS") <> "U"
			cMsgAuxDf	+= " Retido PIS: " + Transform(Val(oTotal:_retTrib:_vRetPIS:TEXT),"@e 9,999,999,999,999.99")
		Endif 
		If Type("oTotal:_retTrib:_vRetCOFINS") <> "U"
			cMsgAuxDf	+= " Retido COFINS: " + Transform(Val(oTotal:_retTrib:_vRetCOFINS:TEXT),"@e 9,999,999,999,999.99")
		Endif 
		If Type("oTotal:_retTrib:_vRetCSLL") <> "U"
			cMsgAuxDf	+= " Retido CSLL: " + Transform(Val(oTotal:_retTrib:_vRetCSLL:TEXT),"@e 9,999,999,999,999.99")
		Endif 
		If Type("oTotal:_retTrib:_vBCIRRF") <> "U"
			cMsgAuxDf	+= " Base IRRF: " + Transform(Val(oTotal:_retTrib:_vBCIRRF:TEXT),"@e 9,999,999,999,999.99")
		Endif 
		If Type("oTotal:_retTrib:_vIRRF") <> "U"
			cMsgAuxDf	+= " Retido IRRF: " + Transform(Val(oTotal:_retTrib:_vIRRF:TEXT),"@e 9,999,999,999,999.99")
		Endif 
...
For nZ := 1 To nLenDet
	...
	//(+------------------------------------------------
	// Customização Atrialub 
	// Aumenta o tamanho do código do produto que pode ser impresso por linha evitando quebra de linha
	...
	Endif
		...
		If  lAgltPrdLt
			...
			AllTrim(TransForm(nPIPI,"@r 99.99%")),;
			StrZero( ++nSequencia, 4 ),;
			{nQtd,nVTotal,nBaseICM,nValICM,nValIPI};
			})
		Endif
		// Fim trecho customizado
	...
	//(+------------------------------------------------
	// Customização Atrialub
	// Retirada a verificação lImpAnfav lImpInfAd para permitir a impressão de dados sobre Tambores 
	...
	If (Type("oNf:_infnfe:_det[nPrivate]:_Infadprod:TEXT") <> "U" .Or. Type("oNf:_infnfe:_det:_Infadprod:TEXT") <> "U")	
		//.And. ( lImpAnfav .Or. lImpInfAd ) // Comentado este trecho para ser validado abaixo ao comparar string 'Etiqueta Tambor'
		...
		// Customização Atrialub 
		// Verifica se tem o Texto 'Etiqueta Tambor ou validação padrão se imprime informações adicionais ou Anfavea'
		If  (!lImpInfAd .And. "Etiqueta Tambor:" $ cAux ) .Or. ( lImpAnfav .Or. lImpInfAd )
...
Endif
	...
	If lAgltPrdLt
		If 	Len( aItensAglt ) > 0
			...
			aItensAglt	:= aSort( aItensAglt,,, { |x,y| x[15] < y[15] } )

			nSubTotal	:= 0

			aItens		:= {}
			cProdAgl	:= ""
			cProdLote	:= "" //aItensAux[1,1]
			
			
			For nX := 1 To Len( aItensAglt )

				aArray		:= ARRAY(14)

				aArray[01]	:= aItensAglt[nX,01]
				aArray[02]	:= aItensAglt[nX,02]
				aArray[03]	:= aItensAglt[nX,03]
				aArray[04]	:= aItensAglt[nX,04]

				If Empty( aItensAglt[nX,03] ) .Or. aItensAglt[nX,03] == "-"
					aArray[05] := ""
				Else
					aArray[05] := aItensAglt[nX,05]
				Endif

				aArray[06]	:= aItensAglt[nX,06]
				aArray[07]	:= aItensAglt[nX,16,1] //{nQtd,nVTotal,nBaseICM,nValICM,nValIPI};
				aArray[08]	:= aItensAglt[nX,08]
				aArray[09]	:= aItensAglt[nX,16,2]
				aArray[10]	:= aItensAglt[nX,16,3]
				aArray[11]	:= aItensAglt[nX,16,4]
				aArray[12]	:= aItensAglt[nX,16,5]
				aArray[13]	:= aItensAglt[nX,13]
				aArray[14]	:= aItensAglt[nX,14]
				
			
				If !Empty(aItensAglt[nX,1]) // Existe código do produto
					If aItensAglt[nX,1] <> cProdAgl
						aadd( aItens, {; 
						aArray[01],;
						aArray[02],;
						aArray[03],;
						...
						cProdLote	:= "" // Quando mudar de produto zera a variável de lote para força a adição na proxima linha
						nLastLin	:= Len(aItens)
						cProdAgl 	:= aItensAglt[nX,1]
						
					Else
						
						cProdAgl 	:= aItensAglt[nX,1]
						
						aItens[nLastLin,07]	+= aArray[07]
						aItens[nLastLin,09]	+= aArray[09]
						aItens[nLastLin,10]	+= aArray[10]
						aItens[nLastLin,11]	+= aArray[11]
						aItens[nLastLin,12]	+= aArray[12]
					Endif
					
				Else
				
					// Existe a descrição de lote 
					If "LOTE:" $ aItensAglt[nX,2]  .And. aItensAglt[nX,2] <> cProdLote 
			...
			For nX := 1 To Len(aItens)
				If !Empty(aItens[nX,1])
					aItens[nX,7]	:=  AllTrim(TransForm(aItens[nX,7],TM(aItens[nX,7],TamSX3("D2_QUANT")[1],TamSX3("D2_QUANT")[2])))
					aItens[nX,9]	:=  AllTrim(TransForm(aItens[nX,9],TM(aItens[nX,9],TamSX3("D2_TOTAL")[1],TamSX3("D2_TOTAL")[2])))
					aItens[nX,10]	:=  AllTrim(TransForm(aItens[nX,10],TM(aItens[nX,10],TamSX3("D2_BASEICM")[1],TamSX3("D2_BASEICM")[2])))
					aItens[nX,11]	:=  AllTrim(TransForm(aItens[nX,11],TM(aItens[nX,11],TamSX3("D2_VALICM")[1],TamSX3("D2_VALICM")[2])))
					aItens[nX,12]	:=  AllTrim(TransForm(aItens[nX,12],TM(aItens[nX,12],TamSX3("D2_BASEIPI")[1],TamSX3("D2_BASEIPI")[2])))
				Endif		
			Next 
...
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
	...
	// Customização Atrialub 
	// Efetua verificacao se é reimpressao ou impressão original
	If __cUserId $ GetNewPar("GM_USRPTDF",__cUserId)
		...
		oDanfe:Box(009,000,IIf(cEmpAnt == "05",060,037),101)
		oDanfe:Say(017, 002, "DATA DE RECEBIMENTO", oFont07N:oFont)
		oDanfe:Box(009,100,IIf(cEmpAnt == "05",060,037),500)
		oDanfe:Say(023, 102, "IDENTIFICAÇÃO(NOME LEGÍVEL E CARIMBO) E ASSINATURA DO RECEBEDOR", oFont08N:oFont)
		oDanfe:Box(000,500,IIf(cEmpAnt == "05",060,037),603)
		...
		// Alterada a fonte do Numero da Nota no Canhoto para melhor visualizacao	
		oDanfe:Say(018, 510, "N. "+StrZero(Val(oIdent:_NNf:Text),9), oFont18N:oFont)
		oDanfe:Say(031, 510, "SÉRIE "+oIdent:_Serie:Text, oFont08:oFont)
		If oIdent:_TpNf:Text == "1"
			DbSelectArea("SD2")
			DbSetOrder(3)
			DbSeek(xFilial("SD2")+Padr(Right(StrZero(Val(oIdent:_NNf:Text),9),Len(Alltrim(SD2->D2_DOC))),9)+oIdent:_Serie:Text)

			// ------------------------------------------------------------------------------------------------
			// Trecho Customizado		
			oDanfe:Say(036,420," Ped:"+SD2->D2_PEDIDO,oFont18n:oFont)     
			
			oDanfe:Say(015, 102, "Emissão: "+ Iif(oNF:_INFNFE:_VERSAO:TEXT >= "3.10",ConvDate(oIdent:_DHEmi:TEXT),ConvDate(oIdent:_DEmi:TEXT)) + " Destinatário: "+ NoChar(oDestino:_XNome:TEXT,lConverte)+ " Valor Total R$ " + Alltrim(aTotais[11]), oFont07:oFont)
			
			If !SD2->D2_TIPO $ "DB"
				DbSelectArea("SA1")
				DbSetOrder(1)
				DbSeek(xFilial("SA1")+SD2->(D2_CLIENTE+D2_LOJA))	
				// Customização BRLub - JEAN - 15/Julho/2022 - Adiciona o nome fantasia do cliente para ser impresso no danfe
				oDanfe:Say(023, 335, "*** N.Fant:" + Alltrim(SA1->A1_NREDUZ)+"***", oFont09N:oFont)
			Endif				

		Endif
	Else
		oDanfe:Say(015, 002, "Esta impressão não é o Danfe Original emitido pelo setor de Faturamento da Empresa", oFont18N:oFont)
		oDanfe:Say(030, 002, "Se for necessário o Danfe Original deverá ser solicitado ao Faturista", oFont18N:oFont)	
	Endif
...
// Customizacão BrLub - JEAN - 15/julho/2022
oDanfe:Box(042,000,137,250)
oDanfe:Say(052,098, "Identificação do emitente",oFont10N:oFont)
nLinCalc	:=	065

cStrAux		:=	AllTrim(NoChar(oEmitente:_xNome:Text,lConverte))
aStrAux     := StrTokArr( cStrAux, ' ' )
cImpresso   := ""
for nX := 1 to len( aStrAux )
	if ( Len( cImpresso ) + len( aStrAux[nX] ) ) > nTamNome
		oDanfe:Say(nLinCalc,098,cImpresso, oFont12N:oFont )
		cImpresso := ""
		nLinCalc  += 10
	endif
	cImpresso += aStrAux[nX] +" "
	if nX == len( aStrAux ) .and. !Empty( cImpresso )
		oDanfe:Say(nLinCalc,098,cImpresso, oFont12N:oFont )
		cImpresso := ""
		nLinCalc  += 10
	endif
...
// ------------------------------------------------------------------------------------------------
// Customização Atrialub - Alterando o divisor por 45 caracteres por linha
...
// ------------------------------------------------------------------------------------------------
// Customização BRLub - JEAN - 15/julho/2022 - Adicional para informar o email para contato de nota fiscal eletronica
cStrAux := AllTrim( SuperGetMv( 'MV_X_EMNFE',,'nfe@brlub.com.br' ) )
if !Empty( cStrAux )
	cStrAux := 'E-mail: ' + cStrAux
...
If Type("oEmitente:_EnderEmit:_xCpl") <> "U"
	...
	// ------------------------------------------------------------------------------------------------
	// Customização Atrialub - Adicional para informar o telefone após a cidade, pois  a adição do email, empurrou uma linha para baixo o texto
	/*
	...
	*/                                                                                                                                                      
	oDanfe:Say(nLinCalc,098, oEmitente:_EnderEmit:_xMun:Text+"/"+oEmitente:_EnderEmit:_UF:Text +;
		iif( !Empty( cFoneCont ), ' Fone: ', '' ) + cFoneCont,oFont08N:oFont )
	nLinCalc+= 10
	// ------------------------------------------------------------------------------------------------
...
If nfolha==1
	If lMv_Logod
		If cEmpAnt == "05"
			oDanfe:SayBitmap(069,005,cLogoD,060,068)
		Else
			oDanfe:SayBitmap(045,001,cLogoD,080,090)
		Endif
	Else
		If cEmpAnt == "05"
			oDanfe:SayBitmap(069,005,cLogo,060,068)
		Else
...
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
If cEmpAnt == "05"
	oDanfe:Box(068,350,098,603)
	oDanfe:Box(099,350,115,603)
	oDanfe:Say(112,355,TransForm(SubStr(oNF:_InfNfe:_ID:Text,4),"@r 9999 9999 9999 9999 9999 9999 9999 9999 9999 9999 9999"),oFont12N:oFont)
	oDanfe:Box(116,350,137,603)
	
	If nFolha == 1
		oDanfe:Say(105,355,"CHAVE DE ACESSO DA NF-E",oFont07:oFont)
		nFontSize := 28
		oDanfe:Code128C(097,370,SubStr(oNF:_InfNfe:_ID:Text,4), nFontSize )
	EndIf
Else
...
If Empty(cCodAutDPEC)
	If Empty(cChaveCont)
		oDanfe:Say(Iif(cEmpAnt == "05",125,117),355,"Consulta de autenticidade no portal nacional da NF-e",oFont12:oFont)
		oDanfe:Say(IIf(cEmpAnt == "05",133,127),355,"www.nfe.fazenda.gov.br/portal ou no site da SEFAZ Autorizada",oFont12:oFont)
	Endif
Endif
